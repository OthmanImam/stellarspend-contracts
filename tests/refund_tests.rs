...
use stellar_spend_contracts::contracts::refunds::{
    RefundsContract, RefundRequest, RefundStatus, RefundConfig, RefundEligibility,
    DataKey, RefundEvents
};
...
fn test_pending_refund_request() {
    ...
    // Set user balance for refund
    env.storage().persistent().set(&DataKey::Balance(user.clone()), &15000i128);
...
fn test_refund_rejection() {
    ...
    RefundsContract::reject_refund(env.clone(), admin.clone(), refund_id, rejection_reason.clone());
...
fn test_refund_processing() {
    ...
    let final_balance = env.storage().persistent().get(&DataKey::Balance(user.clone())).unwrap();
...
fn test_double_refund_prevention() {
    ...
    env.storage().persistent().set(&DataKey::Balance(user.clone()), &2000i128);
...
fn test_refund_eligibility_check() {
    ...
    let eligibility = RefundsContract::check_refund_eligibility(
        env.clone(),
        &user,
        1u64,
        500i128,
...
fn test_get_pending_refunds() {
    ...
    assert_eq!(pending_refunds.get(0).unwrap().status, RefundStatus::Pending);
}
